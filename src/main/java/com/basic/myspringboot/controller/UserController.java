...
public class UserController {
    ...
    //입력항목 검증을 하고, 등록처리하는 메서드
    @PostMapping("/adduser")
    public String addUser(@Valid UserReqDTO user, BindingResult result, Model model) {  //@valid 뒤에 BindingResult를 써야함
        //입력항목 검증 오류가 발생했나?
        if (result.hasErrors()) {
            return "add-user";
        }
...
//        return "index";
    ...
    public String updateUser(@PathVariable("id") Long id, @Valid UserReqForm user, BindingResult result, Model model) {
        if (result.hasErrors()) {
            System.out.println(">>>> hasError user " + user);
